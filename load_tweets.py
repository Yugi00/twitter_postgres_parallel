...
def insert_tweet(connection,tweet):
    ...
    with connection.begin() as trans:
        ...
        sql=sqlalchemy.sql.text('''select id_tweets
        from tweets
        ...
        sql = sqlalchemy.sql.text('''insert into users (id_users, created_at, updated_at, id_urls, friends_count, listed_count, favourites_count, statuses_count, protected, verified, screen_name, name, location, description, withheld_in_countries)
        ...
        on conflict do nothing;
            ''')
        connection.execute(sql, {'id_users': tweet['user']['id'], 'created_at': tweet['user']['created_at'], 'updated_at': tweet['created_at'], 'screen_name': remove_nulls(tweet['user']['screen_name']), 'name': remove_nulls(tweet['user']['name']), 'location': remove_nulls(tweet['user']['location']), 'id_urls': user_id_urls, 'description': remove_nulls(tweet['user']['description']), 'protected': tweet['user']['protected'], 'verified': tweet['user']['verified'], 'friends_count': tweet['user']['friends_count'], 'listed_count': tweet['user']['listed_count'], 'favourites_count': tweet['user']['favourites_count'], 'statuses_count': tweet['user']['statuses_count'], 'withheld_in_countries': tweet['user'].get('withheld_in_countries', None) })
        ...
        if tweet.get('in_reply_to_user_id',None) is not None:
            sql=sqlalchemy.sql.text('''insert into users (id_users)
            values (:id_users)
            on conflict do nothing
                ''')
        ...
        # insert the tweet
        sql=sqlalchemy.sql.text(f'''insert into tweets (id_tweets, id_users, created_at, in_reply_to_status_id, in_reply_to_user_id, quoted_status_id, retweet_count, favorite_count, quote_count, withheld_copyright, withheld_in_countries, source, text, country_code, state_code, lang, place_name, geo)
        ...
        on conflict do nothing;
            ''')
        res = connection.execute(sql, {'id_tweets': tweet['id'], 'id_users': tweet['user']['id'], 'created_at': tweet['created_at'], 'in_reply_to_status_id': tweet.get('in_reply_to_status_id', None), 'in_reply_to_user_id': tweet.get('in_reply_to_user_id', None), 'quoted_status_id': tweet.get('quoted_status_id', None), 'retweet_count': tweet.get('retweet_count', None), 'favorite_count': tweet.get('favorite_count', None), 'quote_count': tweet.get('quote_count', None), 'withheld_copyright': tweet.get('withheld_copyright', None), 'withheld_in_countries': tweet.get('withheld_in_countries', None), 'source': remove_nulls(tweet.get('source', None)), 'text': remove_nulls(text), 'country_code': country_code, 'state_code': state_code, 'lang': remove_nulls(tweet.get('lang', None)), 'place_name': remove_nulls(place_name), 'geo': None})
        ...
        for url in urls:
            ...
            sql=sqlalchemy.sql.text('''INSERT INTO tweet_urls (id_tweets, id_urls)
        ...
        for tag in tags:
            sql=sqlalchemy.sql.text('''insert into tweet_tags (id_tweets, tag)
        ...
        for medium in media:
            ...
            sql=sqlalchemy.sql.text('''insert into tweet_media (id_tweets, id_urls, type)
